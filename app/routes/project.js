...
router.get('/', function (req, res, next) {
    ...
    const config = {
        method: 'get',
        url: 'https://iseto-api.wizeflow.io/smartlinks',
        headers: {
            ...data.getHeaders(),
    ...
    axios(config)
        .then(function (response) {
            ...
            res.render('project', { sl: response.data.data });
        })
        .catch(function (error) {
            console.log(error);
            res.render('project', { sl: "" });
        });
...
router.post('/select', function (req, res, next) {
    ...
    res.render('sl', { project_name: req.body.title, sl_id: req.body.id, result: "", code: "", message: "" });
...
router.post('/select/*', function (req, res, next) {
    ...
    axios(config)
        .then(function (response) {
            ...
            res.render('sl', { token: req.session._id, project_name: req.body.title, sl_id: req.body.sl, result: data, code: "200", message: "{status:success, sl:" + req.body.sl + ", from:" + req.body.from + ", to:" + req.body.to + " }" });
        })
        .catch(function (error) {
            console.log(error);
            res.render('sl', { token: req.session._id, project_name: req.body.title, sl_id: req.body.id, result: "", code: "900", message: error });
        });

});

router.post('/select/download', function (req, res, next) {
